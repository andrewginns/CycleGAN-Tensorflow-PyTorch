...
conv2 = partial(slim.separable_conv2d, activation_fn=None)
...
#    Depthwise seperable convolution
    depth_conv = partial(conv2, normalizer_fn=bn, activation_fn=lrelu, biases_initializer=None)

    with tf.variable_scope(scope + '_discriminator', reuse=tf.AUTO_REUSE):
        net = lrelu(conv(img, dim, 4, 2))
        net = depth_conv(net, None, dim * 2, 4, 2)
        net = depth_conv(net, None, dim * 4, 4, 2)
        net = depth_conv(net, None, dim * 8, 4, 1)
        net = conv(net, 1, 4, 1)
...
def generator(img, scope, dim=64, train=True):
    ...
    # conv_bn_relu = partial(conv, depth_multiplier=1, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
